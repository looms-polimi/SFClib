within SFC.Tests;
////esempio problema dell'alternative: bisogna mettere delle restrizioni sia alle condizioni che a condizioni più tempi di ciclo
model ex_001 "Alternative Split/Join test with two branches"
  Boolean res = Cpulse1.y and Cpulse2.y;
  Boolean status "0 idle, 1 firing OUT1 or OUT2";
  ...
  Modelica.Blocks.Sources.BooleanPulse Cpulse1(period = 4, startTime = 1, width = 30) annotation(
  ...
  SFC.BasicBlocks.Condition C2(y = S1.t > 0.5) annotation(
    Placement(visible = true, transformation(origin = {-20, 8}, extent = {{-16, -10}, {16, 10}}, rotation = 0)));
  ...
  SFC.BasicBlocks.Transition T3(Tcycle = 0.3)  annotation(
    Placement(visible = true, transformation(origin = {24, 48}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  SFC.BasicBlocks.Transition T5(Tcycle = 0)  annotation(
  ...
  Modelica.Blocks.Sources.BooleanPulse Cpulse2(period = 2, startTime = 1, width = 30) annotation(
    Placement(visible = true, transformation(origin = {-42, 78}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  SFC.BasicBlocks.Condition C1(y = S0.t > 1) annotation(
    Placement(visible = true, transformation(origin = {-23, 48}, extent = {{-13, -10}, {13, 10}}, rotation = 0)));
  SFC.BasicBlocks.Condition C3(y = S0.t > 2) annotation(
    Placement(visible = true, transformation(origin = {58, 48}, extent = {{-14, -10}, {14, 10}}, rotation = 0)));
  SFC.BasicBlocks.Condition C25(y = S2.t > 1) annotation(
    Placement(visible = true, transformation(origin = {-22, -32}, extent = {{-14, -10}, {14, 10}}, rotation = 0)));
...
equation
  ...
  connect(T25.C, C25.y) annotation(
    Line(points = {{-44, -32}, {-37, -32}}, color = {255, 0, 255}));
  connect(T3.C, C3.y) annotation(
    Line(points = {{36, 48}, {43, 48}}, color = {255, 0, 255}));
  ...
  connect(T2.C, C2.y) annotation(
    Line(points = {{-44, 8}, {-38, 8}}, color = {255, 0, 255}));
...
algorithm
when initial() then
if alternativeSplit21.IN.active then 
status := true; 
else
status := false;  
end if; 
end when;
  when status == false and change(alternativeSplit21.OUT1.fire) then
    status := true;
  end when;
//OUT2.active := false;
//IN.fire     := OUT1.fire;
//when status == false and alternativeSplit21.OUT2.fire then
//  //OUT1.active := false;
//  //IN.fire     := OUT2.fire;
//  status      := true;
//end when;
  when pre(alternativeSplit21.IN.active) == true and alternativeSplit21.IN.active == false and status then
    status := false;
  end when;
//IN.fire     := false;
  annotation(
