...
model testAction_R1 "Action with R qualifier - connected to an Initial Step"
  ...
  SFC.BasicBlocks.Action A(initialStepAction = true, qualifierType = SFC.Types.Qualifier.R) annotation(
...
equation
  connect(T2.OUT, S0.IN) annotation(
    Line(points = {{-10, -22}, {-10, -34}, {-36, -34}, {-36, 88}, {-10, 88}, {-10, 80}}));
...
annotation(
    Documentation(info = "<html><head></head><body><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><div><div>Action&nbsp;<b>A</b>&nbsp;is connected to Step&nbsp;<u>S0</u>. When S0 becomes active, action A is reset and remains reset until the end of the simulation. Obviously the reset operation is not visible without performing previously a set operation, then signal A remains always to zero (see testMergeActions_SR1 to see the SR evolution af an action).&nbsp;</div><div></div></div><div><br></div><div><br></div>
