...
model testActionMI_P "Set/Reset Action with a single set signal and reset signal"
  ...
  SFC.SFCelements.ActionMI_P actionMI_P0(n = 3)   annotation(
    Placement(visible = true, transformation(origin = {-28, 112}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  SFC.SFCelements.ActionMI_P actionMI_P1 annotation(
...
equation
  ...
  connect(S00.X, actionMI_P0.IN[1]) annotation(
    Line(points = {{-94, 112}, {-42, 112}}));
  connect(S01.X, actionMI_P0.IN[2]) annotation(
    Line(points = {{-94, 46}, {-50, 46}, {-50, 112}, {-42, 112}}));
  connect(S03.X, actionMI_P0.IN[3]) annotation(
    Line(points = {{-94, -72}, {-50, -72}, {-50, 112}, {-42, 112}}));
  connect(S11.X, actionMI_P1.IN[1]) annotation(
    Line(points = {{94, 52}, {182, 52}}));
  connect(S12.X, actionMI_P1.IN[2]) annotation(
