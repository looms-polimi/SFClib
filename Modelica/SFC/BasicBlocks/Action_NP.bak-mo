...
model Action_NP "Action performed during a Step of an SFC"
  ...
  SFC.Interfaces.actionIN IN annotation(
...
algorithm
  when initial() then
    if IN.InitialStep then
      if qualifierType == SFC.Types.Qualifier.P then 
        out := false;
        counterP := 1; 
      elseif qualifierType == SFC.Types.Qualifier.N then 
        out := true;
      end if;
    elseif not IN.InitialStep then
       out := false;
       counterP := 0; 
    end if;
  end when;
  ...
  when change(IN.phase_act) then
    ...
    if edge(IN.phase_act) and qualifierType == SFC.Types.Qualifier.N then
      out := IN.phase_act;
    elseif edge(IN.phase_act) and  qualifierType == SFC.Types.Qualifier.P then
      counterP   := counterP + 1;
      out := false;
    end if;
    
    if (pre(IN.phase_act) == true and IN.phase_act == false and qualifierType == SFC.Types.Qualifier.N) then 
    out := false;
    end if;
  end when;
  annotation(
