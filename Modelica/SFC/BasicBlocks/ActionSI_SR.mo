...
model ActionSI_SR "Action Set-Reset associated in different SFC Steps - Single Input"
  ...
  SFC.Interfaces.actionIN IN_S annotation(
  ...
  SFC.Interfaces.actionIN IN_R annotation(
...
when edge(IN_S.phase_act) then
  out := true;
  counterS := counterS + 1;
end when;
  
when edge(IN_R.phase_act) then
  out := false;  
  counterR := counterR + 1;
end when;
  ...
  annotation(
    Icon(graphics = {Line(origin = {-36, 0.5}, points = {{0, 39.5}, {0, -40.5}, {0, -38.5}}), Rectangle(fillColor = {218, 218, 218}, fillPattern = FillPattern.Solid, extent = {{-140, 40}, {100, -40}}), Line(origin = {-36, 0.5}, points = {{0, 39.5}, {0, -40.5}, {0, -38.5}})}),
